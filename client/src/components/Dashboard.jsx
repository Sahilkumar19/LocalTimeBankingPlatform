...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const userResponse = await axios.get('https://localtimebankingplatform-m2jh.onrender.com/api/users/me', { headers });
  ...
  const handleAddService = async (e) => {
    ...
    try {
      const headers = { Authorization: `Bearer ${localStorage.getItem('token')}` };
      const response = await axios.post('https://localtimebankingplatform-m2jh.onrender.com/api/services/add', newService, { headers });
